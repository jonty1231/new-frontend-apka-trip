              ...
              <div className=" rounded-sm border  px-3 py-4 relative space-y-5">
                ...
                <div className="flex items-center gap-3 text-md md:text-xl">
                  ...
                  <div className="flex items-center gap-1">
                    <h4 className="">{fdatas?.Segments[0][0]?.Origin?.Airport?.CityName} - {fdatas?.Segments[0][0]?.Destination?.Airport?.CityName}  </h4>
                    <p className="border-s-2 border-black px-2  text-sm">
                    {formatDate(fdatas?.Segments[0][0]?.Origin?.DepTime)}
                    </p>
                  </div>
                </div>
                <div className="flex  gap-5 flex-col  md:flex-row items-start  justify-between space-y-4 lg:space-y-0 lg:space-x-4">
                  <div className="flex items-center gap-4 ">
                    <img
                   src={`/images/${fdatas?.Segments[0][0]?.Airline?.AirlineCode}.png`}
                      alt=""
                      className="h-10 w-10 rounded-lg"
                    />
                    <div>
                    <p className="text-sm md:text-lg">{fdatas?.Segments[0][0]?.Airline?.AirlineName}</p>
                      <p className="text-xs">{fdatas?.Airline?.Segments[0][0]?.AirlineCode}-{fdatas?.Segments[0][0]?.Airline?.FlightNumber}</p>
                      <p className="text-xs">{[
                        "All",
                        "Economy",
                        "PremiumEconomy",
                        ...
                        "PremiumBusiness",
                        "First",
                      ].filter((inf, ind)=>{ind + 1 == fdatas?.Segments[0][0]?.CabinClass})}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex  w-full  gap-2 justify-between md:w-[70%] md:px-3">
                    <div className="flex flex-col gap-1  items-start">
                      ...
                      <div className="flex flex-col text-xs ">
                        ...
                        {fdatas?.Segments[0][0]?.Origin?.Airport?.CityName}  ({fdatas?.Segments[0][0]?.Origin?.Airport?.AirportCode} )
                        </span>
                        <span>{ formatDate(fdatas?.Segments[0][0]?.Origin?.DepTime)}</span>
                        <span>Terminal -1</span>
                      </div>
                    </div>

                    <div className="flex  flex-col gap-4 items-center">
                      <p className="text-xs">{ Math.floor(fdatas?.Segments[0][0]?.Duration / 60)}h-{fdatas?.Segments[0][0]?.Duration % 60}m</p>
                      ...
                      {fdatas?.IsRefundable?  <span className="border border-green-400 px-6 md:px-8 m-0 py-1 rounded-full font-bold text-[0.5rem]">
                        REFUNDABLE
                    ...
                    <div className="flex flex-col gap-1 items-start">
                      ...
                      <div className="flex flex-col text-xs ">
                        ...
                        {fdatas?.Segments[0][0]?.Destination?.Airport?.CityName}  ({fdatas?.Segments[0][0]?.Destination?.Airport?.AirportCode} )
                        </span>
                        <span>{formatDate(fdatas?.Segments[0][0]?.Destination?.ArrTime)}</span>
                  ...
                  <div className="flex justify-between items-start p-2">
                    <div className="flex justify-start items-center w-[35%] gap-6">
                      ...
                      <div className="">
                        ...
                        {fdatas?.Segments[0][0]?.Airline?.AirlineName}
                        </h6>
                        <p className="text-gray-500 text-[12px] font-semibold">
                        {fdatas?.Segments[0][0]?.Airline?.AirlineCode}-{fdatas?.Segments[0][0]?.Airline?.FlightNumber}
                        </p>
                      </div>
                    </div>
                    <div className="text-black text-sm font-semibold capitalize  w-[28%]">
                     {fdatas?.Segments[0][0]?.Baggage}
                    </div>
                    <div className="text-black text-sm font-semibold capitalize w-[20%]">
                    {fdatas?.Segments[0][0]?.CabinBaggage}
                    </div>
                  </div>
                </div>
                ...
                <div >
                  <div className="flex items-center justify-between p-4">
                    <div className="flex gap-4 items-center">
                      ...
                      <h3 className="text-lg font-semibold">Mr {user?.name}</h3>
                    </div>
            ...
            cursor-pointer transition`}
              ...
              <div className="w-full border border-slate-100 rounded-md shadow-md p-4 mt-4 max-w-lg mx-auto">
                ...
                <div className="mb-6 flex justify-between items-center text-lg font-semibold">
                  ...
                  <span className="text-xl text-[#DA5200]"> ₹  {fdatas?.Fare?.PublishedFare}</span>
                  </div>
                  <button
                ...
                <div>
                <span className="text-gray-500 font-normal text-xs">By Continuing, you agree to the Rules,<span className="text-blue-600"> Privacy Policy</span>,<span className="text-blue-600"> User Agreement</span>,<span className="text-blue-600">Terms & Conditions</span> of NextGen </span>
                </div>
              </div>
